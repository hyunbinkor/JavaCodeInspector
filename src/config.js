...
export const OLLAMA_OPTIMIZATION = {
  TOKEN_ADJUSTMENT: {
    SMALL_REQUEST: { threshold: 1000, tokens: 2000 },
    MEDIUM_REQUEST: { threshold: 5000, tokens: 1500 },
    LARGE_REQUEST: { threshold: 10000, tokens: 1000 },
    XLARGE_REQUEST: { threshold: 15000, tokens: 500 }
  },
...
export function validateConfig() {
  ...
  // 환경 로깅
  logger.info(`🌍 현재 환경: ${config.environment}`);
  
  // LLM Provider 검증
  if (!['ollama', 'bedrock', 'vllm'].includes(config.llm.provider)) {
    errors.push(`지원하지 않는 LLM Provider: ${config.llm.provider}`);
  }
  ...
  if (config.llm.provider === 'vllm') {
    if (!config.llm.vllm.baseUrl) {
      errors.push('vLLM baseUrl이 설정되지 않았습니다');
    }
    if (!config.llm.vllm.model) {
      errors.push('vLLM model이 설정되지 않았습니다');
    }
  }
  ...
  // Vector DB Provider 검증
  if (!['weaviate', 'qdrant'].includes(config.vector.provider)) {
    errors.push(`지원하지 않는 Vector DB Provider: ${config.vector.provider}`);
  }
  ...
  // 문서 설정 검증 (신규)
  if (config.document.supportedFormats.length === 0) {
    errors.push('지원 문서 형식이 설정되지 않았습니다');
  }
  
  const validFormats = ['pdf', 'docx'];
  const invalidFormats = config.document.supportedFormats.filter(
    f => !validFormats.includes(f)
  );
  
  if (invalidFormats.length > 0) {
    errors.push(`지원하지 않는 문서 형식: ${invalidFormats.join(', ')}`);
  }
