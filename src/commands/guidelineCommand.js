...
export async function manageGuidelines(options) {
  ...
  if (options.import) {
    logger.info(`가이드라인 가져오기: ${options.import}`);
    ...
    logger.info('가이드라인 가져오기 완료');

  } else if (options.list) {
    logger.info('저장된 가이드라인 룰 목록:');
    ...
    staticRules.forEach((rule, index) => {
      logger.info(`  ${index + 1}. ${rule.id} - ${rule.title} (${rule.category})`);
    });
    ...
    logger.info(`가이드라인 내보내기: ${options.export}`);
    const allRules = {
    ...
    logger.info('가이드라인 내보내기 완료');
  } else {
    logger.info('옵션을 지정해주세요: --import, --list, --export 중 하나');
  }
}