 ...
 * 내부 흐름:
 * 1. pdf/docx 파일 존재 확인
 * 2. GuidelineExtractor 초기화 및 pdf/docx 파싱
 * 3. LLM을 통해 텍스트에서 구조화된 가이드라인 추출
 * 4. JSON 파일로 저장
 * 5. 옵션에 따라 VectorDB에 바로 import
 * 
 * @param {Object} options - CLI 옵션
 * @param {string} options.input - 입력 PDF/docx 파일 경로
 * @param {string} options.output - 출력 JSON 파일 경로
 * @param {boolean} options.importToDb - VectorDB 자동 import 여부
 */
export async function extractGuidelinesFromGuide(options) {
    logger.info('\n=== PDF 가이드라인 추출 시작 ===');
    ...
    try {
        ...
        logger.error(`현재 디렉토리의 파일 목록을 확인하세요:`);
        try {
            const files = await fs.readdir('.');
            logger.info('현재 디렉토리 파일:', files.filter(f => f.endsWith('.pdf')));
        } catch (e) {
            // 무시
        }
        process.exit(1);
    ...
    try {
        ...
        if (extractor.guidelines.length === 0) {
            console.warn('\n⚠️ 추출된 가이드라인이 없습니다.');
            logger.info('extracted_text_debug.txt 파일을 확인해보세요.');

            if (extractor.extractedText) {
                logger.info(`\n추출된 텍스트 샘플 (처음 500자):`);
                logger.info(extractor.extractedText.substring(0, 500));
            }
            return;
        }
        ...
        extractor.guidelines.slice(0, 3).forEach((guideline, idx) => {
            ...
            logger.info(`   설명: ${guideline.description.substring(0, 100)}...`);
        });
        ...
        if (options.importToDb) {
            ...
            logger.info(`✅ VectorDB import 완료: 성공 ${results.success}개, 실패 ${results.failed}개`);
        }
 ...
 * 2. 가이드라인 배열 추출 및 검증
 * 3. 통계 정보 출력
 * 4. dry-run이 아닐 경우 VectorDB에 배치 import
 * 
 * @param {Object} options - CLI 옵션
 * @param {string} options.input - 입력 JSON 파일 경로
 * @param {boolean} options.dryRun - 미리보기 모드 여부
 */
export async function importGuidelinesToVectorDB(options) {
    ...
    let guidelineData;
    try {
        const fileContent = await loadData(options.input, 'rule');
        guidelineData = fileContent;
